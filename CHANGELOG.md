...
- **Fixed MTP file transfer soft reboot** — enabled `CONFIG_DMA_CMA` so the USB gadget driver can allocate contiguous DMA memory for MTP bulk transfers. Without it, DMA allocation faults during file transfer caused kernel panics and soft reboots.
